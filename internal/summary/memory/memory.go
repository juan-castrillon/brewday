...
func (s *SummaryMemoryStore) AddBottling(id string, carbonation, alcohol, sugar, water, temp, vol, time_min float32, sugarType, notes string) error {
	s.lock.Lock()
	defer s.lock.Unlock()
	sum, err := s.getSummary(id)
	if err != nil {
	...
	sum.BottlingInfo.Time = time_min
	return nil
}
