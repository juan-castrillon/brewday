...
- HA Notifier. Works by using Home Assistant (and a device with its companion app) to send notifications via the `notify` service

### Changed

- **Breaking Change** Changed notifier configuration syntax. Now a `type` is expected and under `settings` the specific configuration for the type
